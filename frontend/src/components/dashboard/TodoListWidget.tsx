...
const TodoListWidget = memo(({ habits, onCheckIn }: TodoListWidgetProps) => {
                 ...
                 return (
                    <div key={habit.id} className="group flex items-center justify-between p-3 rounded-2xl hover:bg-gray-50 transition-colors border border-transparent hover:border-gray-100 dark:hover:bg-gray-700/50 dark:hover:border-gray-700">
                        <div className="flex items-center gap-4 flex-1 min-w-0 mr-4">
                            <div className="w-12 h-12 rounded-xl bg-gray-50 flex items-center justify-center text-2xl shadow-sm dark:bg-gray-700 flex-shrink-0">
                                {habit.icon || '📝'}
                            </div>
                            <div className="min-w-0 flex-1">
                                <h4 className={`font-bold text-sm leading-tight ${isCompleted ? 'text-gray-400 line-through decoration-2 decoration-gray-300 dark:decoration-gray-600' : 'text-gray-800 dark:text-gray-200'}`}>
                                    {habit.name}
                                </h4>
